...
if (isset($_POST['etsi'])) {
    ...
    $oppilasid = $_SESSION['oppilasid'];

    $varausalku = $pvm . " " . $aikaalkaa;
    $varausloppu = $pvm . " " . $aikaloppuu;


    $stmt_max = $conn->prepare("SELECT COUNT(*) AS count FROM luokat WHERE tyyppi = ? AND luokkakoko >= ? AND kaytto='oppilas'");
    $stmt_max->bind_param("si", $tyyppi, $maara);
    $stmt_max->execute();
    ...
    if ($row_max['count'] == 0) {
        ...
        $message = "Liian paljon oppilaita valitussa ryhmässä – ei löytynyt sopivaa luokkaa.";
        $available_rooms = [];
        $reservations_for_day = []; 
    } else {
        $stmt = $conn->prepare("
            ...
            AND luokkanumero NOT IN (
                SELECT luokkanumero FROM oppilasvaraukset
        ...
        if (empty($available_rooms)) {
            $message = "Ei vapaita luokkia valitulla ajalla. Tässä päivän varaukset:";

            $stmt_all = $conn->prepare("
                SELECT 
                    v.luokkanumero,
                    'Oppilas' AS varaaja_tyyppi,
                    o.etunimi AS varaaja_nimi,
                    v.varausalku,
                    v.varausloppu
                FROM oppilasvaraukset v
                JOIN oppilas o ON v.oppilasid = o.oppilasid
                WHERE v.varausalku >= ? AND v.varausalku < DATE_ADD(?, INTERVAL 1 DAY)
                ORDER BY varausalku
            ");
            $stmt_all->bind_param("ss", $pvm, $pvm);
            $stmt_all->execute();
            $result_all = $stmt_all->get_result();

            $reservations_for_day = [];
            while ($row = $result_all->fetch_assoc()) {
                $reservations_for_day[] = $row;
            }
            $stmt_all->close();
        }
    }
...
if (isset($_POST['valitse_luokka'])) {
    ...
    $stmt = $conn->prepare("
    INSERT INTO oppilasvaraukset (oppilasid, luokkanumero, henkilomaara, varausalku, varausloppu)
    VALUES (?, ?, ?, ?, ?)
    ");
    $stmt->bind_param("isiss", $oppilasid, $luokkanumero, $maara, $varausalku, $varausloppu);
...
<head>
    ...
    <title>Luokkavarausjärjestelmä - Varaus (Oppilaat)</title>
    ...
    <body>
        <div class="center">
            <h1 class="noto-serif-bengali">VARAUS - OPPILAAT</h1>
            <a class="tasa-orbiter button" href="omat_varaukset_oppilaat.php">OMAT VARAUKSET</a>
        ...
        <div class="container">
            ...
            <?php if (!empty($reservations_for_day)): ?>
                <p class="tasa-orbiter">Päivän varaukset (<?php echo htmlspecialchars($pvm); ?>)</p>
                <table class="tasa-orbiter" border="1" cellpadding="8" cellspacing="0" style="width:100%; border-collapse: collapse; margin-top:10px; border: rgb(65, 65, 65) solid 2px;">
                    ...
                    <?php foreach ($reservations_for_day as $r): ?>
                        <tr>
                            <td><?php echo htmlspecialchars($r['luokkanumero']); ?></td>
                            <td><?php echo htmlspecialchars($r['varaaja_nimi']); ?></td>
                            <td><?php echo htmlspecialchars($r['varaaja_tyyppi']); ?></td>
                            <td><?php echo date('H:i', strtotime($r['varausalku'])); ?></td>
                            <td><?php echo date('H:i', strtotime($r['varausloppu'])); ?></td>
                        </tr>
                    <?php endforeach; ?>
                </table>
            <?php endif; ?>           
